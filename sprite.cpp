...
//‚ ‚é‚Ó‚Ÿ‚Ì‚Ù‚¤
static D3DCOLOR g_Color_Alpha = D3DCOLOR_RGBA(255, 255, 255, 255);
;
//•’Ê
...
//ƒ¿’l‚ª‚ ‚é‚â‚Â
void Sprite_Draw(TextureIndex texture_index, float dx, float dy, int tx, int ty, int tw, int th, float cx, float cy, float sx, float sy, float rotation, int Alpha)
{
	LPDIRECT3DDEVICE9 pDevice = GetD3DDevice();
	if (!pDevice) return;
	...
	g_Color_Alpha = D3DCOLOR_RGBA(255, 255, 255, Alpha);

	Vertex2D vertexes[] = {
		{ D3DXVECTOR4(px[0] - 0.5f, py[0] - 0.5f, 0.0f, 1.0f), g_Color_Alpha, D3DXVECTOR2(u[0], v[0]) },
		{ D3DXVECTOR4(px[1] - 0.5f, py[1] - 0.5f, 0.0f, 1.0f), g_Color_Alpha, D3DXVECTOR2(u[1], v[0]) },
		{ D3DXVECTOR4(px[2] - 0.5f, py[2] - 0.5f, 0.0f, 1.0f), g_Color_Alpha, D3DXVECTOR2(u[0], v[1]) },
		{ D3DXVECTOR4(px[3] - 0.5f, py[3] - 0.5f, 0.0f, 1.0f), g_Color_Alpha, D3DXVECTOR2(u[1], v[1]) },
	};
